...
# TMDC monolayer. We choose the oscillator strength f so that σ_res = A·σ₀.
function exciton_sigma(λ)
    ...
    A  = 20.0               # peak conductivity in units of σ₀ (σ_res ≈ 20 σ₀)
    ω  = 2π * c0 / λ
    ω0 = 2π * c0 / λ0
    Γ  = 2π * c0 * Γλ / λ0^2
...
# is N quarter-wave (hi/lo) periods, giving enough finesse to resolve the polariton
# splitting. The spacer is tuned to λ_cav > λ₀ so the cavity mode starts red of the
# exciton at normal incidence and blueshifts through it as the angle increases,
# tracing a clear avoided crossing. The monolayer sits at the spacer center.
n_air, n_hi, n_lo, n_spacer = 1.0, 2.2, 1.45, 1.5
λ0     = 0.62                  # exciton wavelength (μm)
λ_cav  = 0.645                 # cavity-mode wavelength at normal incidence (μm)
N      = 3                     # quarter-wave periods per mirror
...
front = reduce(vcat, [[mirror_hi, mirror_lo] for _ in 1:N])
back  = reduce(vcat, [[mirror_lo, mirror_hi] for _ in 1:N])
layers = [air, front..., spacer_half, spacer_half, back..., sub]
sheet_index = 1 + 2N + 2                  # interface between the two spacer halves (cavity center)
sheets = Dict(sheet_index => Sheet(exciton_sigma))

λs = range(0.58, 0.66, length = 400)
θs = range(0.0, 0.7, length = 200)
...
z_sheet = ef.boundaries[sheet_index - 1]   # z-position of the conductive sheet
